...
bgd1 = cv2.createBackgroundSubtractorMOG2( #배경제거용
    history = 300,
    varThreshold = 35,
    detectShadows = True
)
...
while True:
    ...
    fgmask = bgd1.apply(frame) # 배경 제거

    _, thresh = cv2.threshold (fgmask, 200, 255, cv2.THRESH_BINARY)
    
    # 모폴로지 작업
    kernel = cv2.getStructuringElement( cv2.MORPH_RECT, (3,3))
    fmask = cv2.morphologyEx( # 노이즈 제거 ( 작은 점 제거)
    ...
    for obj_id in objects: # 현재 프레임에 모든 객체에서 miss 1 증가
    ...
    delete_ids = []
    for obj_id, obj in objects.items(): # 30 프레임이상 다시 움직이지 않으면 삭제
